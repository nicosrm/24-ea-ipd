...
Zellen untergebracht werden. Die Staatsanwaltschaft bietet ihnen das sogenannte
...
Situation, die bei einem IPD auftreten können. Es wurde sich hierbei auf die
drei letzten Züge beschränkt. Somit entsteht ein Dictionary mit $2^3 = 8$
möglichen *Keys* – $\{(C,C,C), (C,C,D), (C,D,C), \ldots\}$ (mit $C$ für
*cooperate* und $D$ für *defect*) – als Bedingungen und entsprechend so vielen
Aktionen als *Value*. In der Implementierung wird dies beispielhaft wie folgt
dargestellt.
...
Strategien gegen ihre Gegner erzielen konnten. Im Code wird diese Art durch die
...
hinzu, welche für jede Bedingung zufällig die Aktion herumdreht [@weicker2015].
...
Beim Start des Programms müssen die Mutations- und Rekombinationsrate als
...
Dabei sei angemerkt, dass die `matchIterationCount` beschreibt, wie viele Runden
pro Spiel, d.h. Strategie vs. Strategie, gespielt werden sollen. Ferner bestimmt
das `tournamentSteps`-Argument das $q$ für die $q$-stufige Turnier-Selektion.
...
bei der etablierten Selektion dahingegen nur sechs.

Die Experimente lassen sich mit dem folgenden Ausführungsbefehl reproduzieren.
...
![Defection Rate im Verlauf der Epochen gegen etablierte Strategien von drei
...
ob dies einen Trend oder einen weiteren Ausreißer darstellt. File 1 und
...
$(C)$ und einmal verraten $(D)$ hat, ist unser Zug Verrat $(D)$.
...
beiden Systeme zu vergleichen war interessant, v.a. da hier zwei verschiedene
Ansätze miteinander konkurrieren. Einerseits die Bewertung anhand von Spielen
...
Vielmehr besteht diese lediglich aus der Anzahl der Siege, welches das jeweilige
